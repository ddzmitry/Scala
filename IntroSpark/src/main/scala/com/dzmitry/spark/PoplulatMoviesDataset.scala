...
object PoplulatMoviesDataset {
  ...
  final case class Movie(movieID: Int, Raiting: Int)
  ...
  def main(args: Array[String]) {
    ...
    val lines = spark.sparkContext.textFile("../ml-100k/u.data").map(x => Movie(x.split("\t")(1).toInt,x.split("\t")(2).toInt))
    ...
    moviesDS.show()
    val columns = Seq[String]("movieID", "Raiting")
